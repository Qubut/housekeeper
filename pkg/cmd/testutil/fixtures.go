	...
	return []MigrationFile{
		{
			...
			SQL: `CREATE DATABASE db ENGINE = Atomic COMMENT 'Sample database';
...
SETTINGS index_granularity = 8192;`,
		},
		{
			...
			SQL: `CREATE DICTIONARY db.user_status_dict (
...
LIFETIME(MIN 300 MAX 3600);`,
		},
		{
			...
			SQL: `CREATE VIEW db.active_users AS
SELECT
    id,
    email,
    name
...
// SampleSchema returns a test sample database schema
...
func UsersTableSchema() string {
	return `CREATE TABLE IF NOT EXISTS db.users (
    id UInt64,
    email String,
    name String,
    created_at DateTime DEFAULT now(),
...
func EventsTableSchema() string {
	return `CREATE TABLE IF NOT EXISTS db.events (
    id UInt64,
    user_id UInt64,
    session_id String,
    event_type LowCardinality(String),
...
func ComplexDictionarySchema() string {
	return `CREATE DICTIONARY IF NOT EXISTS db.geo_data (
    id UInt64 IS_OBJECT_ID,
...
func MaterializedViewSchema() string {
	return `CREATE MATERIALIZED VIEW IF NOT EXISTS db.hourly_metrics
...
func ReplicatedTableSchema() string {
	return `CREATE TABLE IF NOT EXISTS db.replicated_data ON CLUSTER '{cluster}' (
    id UInt64,
    shard_key UInt64,
    data String,
