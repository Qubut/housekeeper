...
func TestParameterExprRoundTripIdempotent(t *testing.T) {
	...
	sql := `CREATE VIEW v_candidates AS ` +
		`SELECT c.item_id AS item_id, w.last_seen AS last_seen ` +
		`FROM (SELECT arrayJoin({ids:Array(UInt64)}) AS item_id) AS c ` +
		`LEFT JOIN (SELECT item_id, max(seen_at) AS last_seen FROM history ` +
		`WHERE source = 'a' AND item_id IN {ids:Array(UInt64)} GROUP BY item_id) AS w ` +
