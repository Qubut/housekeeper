...
func TestCreateDictionary(t *testing.T) {
	...
	tests := []statementTest{
		...
		{name: "if_not_exists_on_cluster", sql: `CREATE DICTIONARY IF NOT EXISTS db.user_mapping ON CLUSTER production (user_id UInt64 IS_OBJECT_ID, user_name String, group_id UInt32 HIERARCHICAL, status String INJECTIVE) PRIMARY KEY user_id, group_id SOURCE(ClickHouse(host 'localhost' port 9000 user 'default' password '' db 'users' table 'mapping')) LAYOUT(COMPLEX_KEY_HASHED()) LIFETIME(MIN 60 MAX 3600) SETTINGS(max_block_size = 8192, max_threads = 2) COMMENT 'Complex user mapping dictionary';`},
		{name: "postgresql_named_collection_nullable", sql: `CREATE DICTIONARY IF NOT EXISTS default.catalog_items ON CLUSTER '{cluster}' (item_id UInt64, exterior Nullable(String), rarity Nullable(String)) PRIMARY KEY item_id SOURCE(POSTGRESQL(NAME postgres_catalog TABLE 'catalog_items' SCHEMA 'counter_strike' where '1 = 1' invalidate_query 'SELECT max(item_id) FROM counter_strike.catalog_items')) LAYOUT(HASHED_ARRAY()) LIFETIME(MIN 300 MAX 600);`},
		...
		{name: "http_with_multiple_headers", sql: `CREATE DICTIONARY sample_dict (id UInt64, data String) PRIMARY KEY id SOURCE(HTTP(url 'https://api.example.com/data' format 'JSONEachRow' credentials(user 'api_user' password 'secret123') headers(header(name 'Content-Type' value 'application/json') header(name 'X-Custom-Header' value 'custom-value')))) LAYOUT(FLAT()) LIFETIME(MIN 300 MAX 1800);`},
...
func TestAttachDictionary(t *testing.T) {
	...
	tests := []statementTest{
		...
		{name: "if_not_exists", sql: `ATTACH DICTIONARY IF NOT EXISTS db.attach_ifnotexists_dict;`},
		{name: "on_cluster", sql: `ATTACH DICTIONARY attach_cluster_dict ON CLUSTER production;`},
		{name: "full_options", sql: `ATTACH DICTIONARY IF NOT EXISTS db.attach_full_dict ON CLUSTER production;`},
	}
...
func TestDetachDictionary(t *testing.T) {
	...
	tests := []statementTest{
		...
		{name: "if_exists", sql: `DETACH DICTIONARY IF EXISTS db.detach_ifexists_dict;`},
		...
		{name: "full_options", sql: `DETACH DICTIONARY IF EXISTS db.detach_full_dict ON CLUSTER production PERMANENTLY SYNC;`},
	}
...
func TestDropDictionary(t *testing.T) {
	...
	tests := []statementTest{
		...
		{name: "if_exists", sql: `DROP DICTIONARY IF EXISTS db.drop_ifexists_dict;`},
		{name: "on_cluster", sql: `DROP DICTIONARY drop_cluster_dict ON CLUSTER production;`},
		{name: "full_options", sql: `DROP DICTIONARY IF EXISTS db.drop_full_dict SYNC;`},
	}
